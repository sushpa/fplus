...
function central2d(Q[:,:] as Real, by[:] as Real, axis as Int) result (dQ[:,:] as Real)
    check 1 <= axis <= dims(Q)
...
function central1d(Q[], by[]) result (dQ[])
...
function boundary2d(q[:,:] as Real, zero as Boolean) result (Qnew[:,:] as Real)

...
static void Parser_errorUnrecognizedMember(
    Parser* const this, const ASTType* const type, const ASTExpr* const expr)
{
...
static void Parser_errorDuplicateType(
    Parser* const this, const ASTType* const type, const ASTType* const orig)
{
    if (orig)
...
static void Parser_errorCtorHasType(
    Parser* const this, const ASTFunc* const func, const ASTType* const orig)
{
...
static void Parser_errorDuplicateFunc(
    Parser* const this, const ASTFunc* const func, const ASTFunc* const orig)
{
...
static void Parser_errorDuplicateTest(
    Parser* const this, const ASTTest* const test, const ASTTest* const orig)
{
    eprintf("\n(%d) \e[31merror:\e[0m duplicate test "
            "\e[34m%s\e[0m at %s%s:%d:%d\n"
            "             already declared at %s%s:%d:%d\n",
        this->errCount + 1, test->name, RELF(this->filename), test->line, 1,
        RELF(this->filename), orig->line, 1);
    Parser_errorIncrement(this);
}
    ...
    Parser* const this, const ASTExpr* const expr)
{
    ...
    eprintf("\n(%d) \e[31merror:\e[0m arg count mismatch for "
        ...
        this->errCount + 1, expr->func->name, RELF(this->filename), expr->line,
        expr->col, ASTExpr_countCommaList(expr->left), expr->func->argCount,
        RELF(this->filename), expr->func->line);
    Parser_errorIncrement(this);
}
...
static void Parser_errorInvalidTestName(Parser* const this)
{
    eprintf("\n(%d) \e[31merror:\e[0m invalid test name "
            "\e[33m%.*s\e[0m at %s%s:%d"
            "\n       test names must be strings\n",
        this->errCount + 1, this->token.matchlen, this->token.pos,
        RELF(this->filename), this->token.line);
    Parser_errorIncrement(this);
}
    ...
    Parser* const this, const ASTExpr* const expr)
{
    eprintf("\n(%d) \e[31merror:\e[0m mutating read-only variable '"
            "\e[34m%s\e[0m' at %s%s:%d:%d\n",
        this->errCount + 1, expr->var->name, RELF(this->filename), expr->line,
        expr->col);
    Parser_errorIncrement(this);
}

...
int main()
{
    ...
    double xnew[jet_countof(x)];
    for (int i = 0; i < jet_countof(x); i++) xnew[i] = x[i] / 2.0;
    // double* xp = x;
    printarrf(x, jet_countof(x));
    printarrf(xnew, jet_countof(x));
...
// Array_initWith_cArray(Number)(x, _1, jet_countof(_1));

// x = [ 1, 2,    cos(x) ]
// -----------------------------------------------
// Number _2[]    = { 1, 2, cos(x) };
// Array_initWith_cArray(Number)(_x, jet_countof(_x));
